...
use crate::{
    ...
    domain::models::{payment::{payment_to_response, Payment, PaymentResponse}, payment_history::{PaymentHistory, PaymentHistoryResponse, StatusPayment}, product::{product_to_response, Product}},
};
...
pub async fn all_payment_history_query(
    ...
    let result_payment_history = sqlx::query_as!(
        PaymentHistory,
        r#"
        SELECT 
        id, user_id, user_amount_money, total_price, status_payment AS "status_payment!: StatusPayment", created_at, updated_at
    ...
    let data = payment_history(result_payment_history,&data).await.map_err(|e|error_response(format!("Failed to get data payment {:?}",e).as_str()))?;
    Ok((
        StatusCode::OK,
